...
public final class EncryptedData {
    ...
    public static EncryptedData readEncryptedData(ByteBuffer buffer, int length, int maxLength)
            throws isoRAWException.NotValidException {
        if (length == 0) {
            return EMPTY_DATA;
        }
        if (length > maxLength) {
            throw new isoRAWException.NotValidException("Max encrypted data length exceeded: " + length);
        }
    ...
    public static EncryptedData readEncryptedData(byte[] bytes) {
        ...
        } catch (isoRAWException.NotValidException e) {
