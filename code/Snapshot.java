...
import isoRaw.Currency;
import isoRaw.Db;
import isoRaw.FxtDistribution;
import isoRaw.Genesis;
...
public class Snapshot implements AddOn {
    ...
    private static final int snapshotHeight = isoRaw.getIntProperty("isoRaw.snapshotHeight", Integer.MAX_VALUE);
    private static final boolean snapshotForTestnet = isoRaw.getBooleanProperty("isoRaw.snapshotForTestnet", true);

    private static final Set<String> scammerAccounts = Collections.unmodifiableSet(new HashSet<>(Arrays.asList(
            Long.toUnsignedString(Convert.parseAccountId("isoRaw-TN8U-RBVE-GBJ3-7DEBN")), // scammer
            Long.toUnsignedString(Convert.parseAccountId("isoRaw-V79Z-RQ5X-XXJR-H8P87")), // scammer
            Long.toUnsignedString(Convert.parseAccountId("isoRaw-BH28-PKY6-LES8-29DBN")), // scammer
            Long.toUnsignedString(Convert.parseAccountId("isoRaw-ZKV3-J2WN-T6VM-B28DA")), // scammer
            Long.toUnsignedString(Convert.parseAccountId("isoRaw-ZPRA-ZDUQ-SYEL-7AAJM")), // scammer
            Long.toUnsignedString(Convert.parseAccountId("isoRaw-UMZH-XLBB-BGSY-7WESP")), // scammer
            Long.toUnsignedString(Convert.parseAccountId("isoRaw-5CFL-QTTH-D6K2-AC4TF")), // scammer
            Long.toUnsignedString(Convert.parseAccountId("isoRaw-B2KJ-DAAF-884G-GENQ8")), // scammer
            Long.toUnsignedString(Convert.parseAccountId("isoRaw-L8JG-U967-NUNS-BA4RK")), // scammer
            Long.toUnsignedString(Convert.parseAccountId("isoRaw-LSMJ-YCH7-QESX-AH42N")), // compromised
            Long.toUnsignedString(Convert.parseAccountId("isoRaw-E8JD-FHKJ-CQ9H-5KGMQ"))  // compromised
    )));

    private static final String wrongBountyAccount = Long.toUnsignedString(Convert.parseAccountId("isoRaw-XTJE-PLDX-EZ6E-6FQX6"));

    private static final String AEUR_ACCOUNT = "isoRaw-NJ92-R5GB-HQB4-6NW7T";

    @Override
    public void init() {

        isoRaw.getBlockchainProcessor().addListener(new Listener<Block>() {
            ...
            private Map<String, Long> exportIgnisBalances() {
                        ...
                        while (rs.next()) {
                            ...
                            if (snapshotForTestnet) {
                                eurSnapshotMap.put(account, Convert.longValueExact(
                                        ...
                                        .divide(BigInteger.valueOf(Constants.ONE_isoRaw))));
                            } else {
                ...
                if (snapshotForTestnet && !developerPublicKeys.isEmpty()) {
                    ...
                    developerPublicKeys.forEach(publicKey -> {
                        ...
                        eurSnapshotMap.put(account, (developerBalance / Constants.ONE_isoRaw) * 10000);
                    });
                }
            ...
            private boolean invalidCurrency(String code, String normalizedName) {
                ...
                if (code.contains("isoRaw") || code.contains("NEXT") || "isoRaw".equals(normalizedName) || "next".equals(normalizedName)) {
                    return true;
                }
