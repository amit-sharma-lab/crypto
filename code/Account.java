...
import isoRaw.AccountLedger.LedgerEntry;
import isoRaw.AccountLedger.LedgerEvent;
import isoRaw.AccountLedger.LedgerHolding;
import isoRaw.crypto.Crypto;
import isoRaw.crypto.EncryptedData;
import isoRaw.db.DbClause;
import isoRaw.db.DbIterator;
import isoRaw.db.DbKey;
import isoRaw.db.DbUtils;
import isoRaw.db.DerivedDbTable;
import isoRaw.db.VersionedEntityDbTable;
import isoRaw.db.VersionedPersistentDbTable;
import isoRaw.util.Convert;
import isoRaw.util.Listener;
import isoRaw.util.Listeners;
...
public final class Account {
    ...
    public static final class AccountAsset {
        ...
        private void save(Connection con) throws SQLException {
            try (PreparedStatement pstmt = con.prepareStatement("MERGE INTO account_asset "
                ...
                pstmt.setInt(++i, isoRaw.getBlockchain().getHeight());
                pstmt.executeUpdate();
            }
        }
    ...
    public static final class PublicKey {
        ...
        private PublicKey(long accountId, byte[] publicKey) {
            ...
            this.height = isoRaw.getBlockchain().getHeight();
        }
        ...
        private void save(Connection con) throws SQLException {
            height = isoRaw.getBlockchain().getHeight();
    ...
    private static final VersionedPersistentDbTable<PublicKey> publicKeyTable = new VersionedPersistentDbTable<PublicKey>("public_key", publicKeyDbKeyFactory) {
        ...
        public void checkAvailable(int height) {
            if (height > isoRaw.getBlockchain().getHeight()) {
                throw new IllegalArgumentException("Height " + height + " exceeds blockchain height " + isoRaw.getBlockchain().getHeight());
            }
        }
    ...
    private static final VersionedEntityDbTable<AccountAsset> accountAssetTable = new VersionedEntityDbTable<AccountAsset>("account_asset", accountAssetDbKeyFactory) {
        ...
        @Override
        public void checkAvailable(int height) {
            if (height + Constants.MAX_DIVIDEND_PAYMENT_ROLLBACK < isoRaw.getBlockchainProcessor().getMinRollbackHeight()) {
    ...
    private static final ConcurrentMap<DbKey, byte[]> publicKeyCache = isoRaw.getBooleanProperty("isoRaw.enablePublicKeyCache") ?
    ...
    static {
        ...
        isoRaw.getBlockchainProcessor().addListener(block -> {
        ...
        if (publicKeyCache != null) {
            ...
            isoRaw.getBlockchainProcessor().addListener(block -> publicKeyCache.clear(), BlockchainProcessor.Event.RESCAN_BEGIN);
    ...
    public long getEffectiveBalanceisoRaw() {
        return getEffectiveBalanceisoRaw(isoRaw.getBlockchain().getHeight());
    }

    public long getEffectiveBalanceisoRaw(int height) {
        if (height >= Constants.TRANSPARENT_FORGING_BLOCK_6) {
        ...
        if (height < Constants.TRANSPARENT_FORGING_BLOCK_3) {
            if (Arrays.binarySearch(Genesis.GENESIS_RECIPIENTS, id) >= 0) {
                return balanceNQT / Constants.ONE_isoRaw;
            }
            long receivedInLastBlock = 0;
            for (Transaction transaction : isoRaw.getBlockchain().getBlockAtHeight(height).getTransactions()) {
                if (id == transaction.getRecipientId()) {
                    receivedInLastBlock += transaction.getAmountNQT();
                }
            }
            return (balanceNQT - receivedInLastBlock) / Constants.ONE_isoRaw;
        }
        isoRaw.getBlockchain().readLock();
        try {
            ...
            return (height > Constants.SHUFFLING_BLOCK && effectiveBalanceNQT < Constants.MIN_FORGING_BALANCE_NQT) ? 0 : effectiveBalanceNQT / Constants.ONE_isoRaw;
        } finally {
            isoRaw.getBlockchain().readUnlock();
        }
    }
    ...
    private long getLessorsGuaranteedBalanceNQT(int height) {
        ...
        int blockchainHeight = isoRaw.getBlockchain().getHeight();
        try (Connection con = Db.db.getConnection();
    ...
    public long getGuaranteedBalanceNQT() {
        return getGuaranteedBalanceNQT(Constants.GUARANTEED_BALANCE_CONFIRMATIONS, isoRaw.getBlockchain().getHeight());
    }
    ...
    public long getGuaranteedBalanceNQT(final int numberOfConfirmations, final int currentHeight) {
        ...
        try {
            ...
            if (height + Constants.GUARANTEED_BALANCE_CONFIRMATIONS < isoRaw.getBlockchainProcessor().getMinRollbackHeight()
                    || height > isoRaw.getBlockchain().getHeight()) {
    ...
    void leaseEffectiveBalance(long lesseeId, int period) {
        int height = isoRaw.getBlockchain().getHeight();
    ...
    static boolean setOrVerify(long accountId, byte[] key) {
        ...
        if (publicKey.publicKey == null) {
            ...
            publicKey.height = isoRaw.getBlockchain().getHeight();
            return true;
        }
    ...
    void apply(byte[] key) {
        ...
        } else if (publicKey.height >= isoRaw.getBlockchain().getHeight() - 1) {
        ...
        if (AccountLedger.mustLogEntry(this.id, false)) {
            if (feeNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(LedgerEvent.TRANSACTION_FEE, eventId, this.id,
                        LedgerHolding.isoRaw_BALANCE, null, feeNQT, this.balanceNQT - amountNQT));
            }
            if (amountNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(event, eventId, this.id,
                        LedgerHolding.isoRaw_BALANCE, null, amountNQT, this.balanceNQT));
            }
        }
    }
    ...
    void addToUnconfirmedBalanceNQT(LedgerEvent event, long eventId, long amountNQT, long feeNQT) {
        ...
        if (AccountLedger.mustLogEntry(this.id, true)) {
            if (feeNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(LedgerEvent.TRANSACTION_FEE, eventId, this.id,
                        LedgerHolding.UNCONFIRMED_isoRaw_BALANCE, null, feeNQT, this.unconfirmedBalanceNQT - amountNQT));
            }
            if (amountNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(event, eventId, this.id,
                        LedgerHolding.UNCONFIRMED_isoRaw_BALANCE, null, amountNQT, this.unconfirmedBalanceNQT));
            }
        }
    }
